...
// Tool model and integration contracts
...
export interface EnsurePluginToolsOptions {
    plugins: Plugin[];
...
const HANDBRAKE_ASSETS: Record<string, HandBrakeAsset> = {
    "darwin-arm64": {
        archive: "dmg",
...
function defaultToolRoot(env: NodeJS.ProcessEnv, platform: NodeJS.Platform): string {
    const home = homedir();
    if (platform === "win32")
        return join(env.LOCALAPPDATA || join(home, "AppData", "Local"), "straif-plugins", "tools");
    if (platform === "darwin") return join(home, "Library", "Application Support", "straif-plugins", "tools");
    return join(env.XDG_DATA_HOME || join(home, ".local", "share"), "straif-plugins", "tools");
}

export function handBrakeResolutionPath(toolRoot: string): string {
    return join(toolRoot, "handbrake-cli", "resolution.json");
}

export function handBrakeAsset(platform: NodeJS.Platform, arch: string): HandBrakeAsset | undefined {
    return HANDBRAKE_ASSETS[`${platform}-${arch}`];
}

function managedExecutablePath(toolRoot: string, platform: NodeJS.Platform, arch: string): string {
...
async function defaultRun(command: string, args: string[], options: RunOptions = {}): Promise<CommandResult> {
...
async function probeHandBrake(
    resolution: Omit<ToolResolution, "version">,
    run: NonNullable<EnsurePluginToolsOptions["run"]>
...
async function writeResolution(toolRoot: string, resolution: ToolResolution): Promise<void> {
...
async function downloadVerified(
    ...
    if (!response.ok || !response.body) throw new Error(`The HandBrake download failed with HTTP ${response.status}.`);
...
async function findNamedFile(directory: string, expectedName: string): Promise<string | undefined> {
...
export async function extractWindowsArchive(archive: string, payload: string): Promise<string> {
    ...
    try {
        for await (const entry of zip.eachEntry()) {
            if (entry.fileName !== "HandBrakeCLI.exe") continue;
            ...
            await pipeline(
                ...
                limiter,
                createWriteStream(payload, { flags: "wx", mode: 0o755 })
            );
...
async function extractMacArchive(
    archive: string,
    payload: string,
    run: NonNullable<EnsurePluginToolsOptions["run"]>
): Promise<string> {
    ...
    try {
        ...
        if (!executable) throw new Error("The HandBrake disk image did not contain HandBrakeCLI.");
...
async function installManagedHandBrake({
    asset,
    platform,
    arch,
    toolRoot,
    ...
    try {
        ...
        if (asset.archive === "zip") await extractWindowsArchive(archive, payload);
        else await extractMacArchive(archive, payload, run);
        ...
        if (!stagedResolution) throw new Error("The downloaded HandBrakeCLI executable could not be started.");

        const finalDirectoryExists = await stat(finalDirectory).then(
            () => true,
            () => false
        );
        ...
        try {
            ...
            if (backup) await rename(backup, finalDirectory);
            throw error;
        }
...
async function ensureHandBrake(options: EnsurePluginToolsOptions): Promise<ToolResolution | undefined> {
    ...
    if (platform === "linux") {
        ...
        if (!flatpak)
            throw new Error(
                `HandBrakeCLI was not found. Install Flatpak for your Linux distribution, then run this installer again. ${HANDBRAKE_DOWNLOAD_PAGE}`
            );
        ...
        const accepted = await options.confirmInstall(
            "MediaCompress requires HandBrakeCLI. Add the official Flathub repository and install HandBrake for your user account?"
        );
        if (!accepted) return undefined;
        const addRemote = await run(
            flatpak,
            ["--user", "remote-add", "--if-not-exists", "flathub", FLATHUB_REPOSITORY],
        ...
        if (install.code !== 0)
            throw new Error(
                `The HandBrake Flatpak could not be installed. Install it manually from: ${HANDBRAKE_DOWNLOAD_PAGE}`
            );
        const resolution = await resolveFlatpak(flatpak, run);
        if (!resolution) throw new Error("The HandBrake Flatpak was installed but HandBrakeCLI could not be started.");
        await writeResolution(toolRoot, resolution);
        return resolution;
    }
    ...
    if (!asset)
        throw new Error(
            `There is no supported portable HandBrakeCLI package for ${platform}-${arch}. Install HandBrakeCLI manually and place it on PATH: ${HANDBRAKE_DOWNLOAD_PAGE}`
        );
    const accepted = await options.confirmInstall(
        `MediaCompress requires HandBrakeCLI. Download the official HandBrakeCLI ${HANDBRAKE_VERSION} package into this private per-user folder? ${toolRoot}`
    );
    if (!accepted) return undefined;
...
export async function ensurePluginTools(
    options: EnsurePluginToolsOptions
): Promise<{ ready: Plugin[]; skipped: string[] }> {
    ...
    if (!requestedTools.has("handbrake-cli")) return { ready: options.plugins, skipped: [] };
...
export async function readHandBrakeResolution(toolRoot: string): Promise<ToolResolution | undefined> {
    try {
        const value = JSON.parse(await readFile(handBrakeResolutionPath(toolRoot), "utf8")) as Partial<ToolResolution>;
        if (
            ...
            return undefined;
        return value as ToolResolution;
    } catch {
        return undefined;
    }
}
