...
const RESERVED_DIRECTORIES: Record<string, true> = {
    cli: true,
    cloned: true,
    node_modules: true,
    test: true
...
interface PrepareCatalogSourceOptions {
    sourceRoot?: string;
    packageRoot?: string;
...
export async function prepareCatalogSource({
    sourceRoot = SOURCE_ROOT,
    packageRoot = PACKAGE_ROOT,
    repositoryUrl = REPOSITORY_URL
}: PrepareCatalogSourceOptions = {}): Promise<PreparedCatalogSource> {
    const isPackagedSnapshot =
        resolve(sourceRoot) === resolve(packageRoot) && !(await gitOutput(["rev-parse", "HEAD"], sourceRoot));
    if (!isPackagedSnapshot) return { root: sourceRoot, cleanup: async () => {} };
    ...
    try {
        await new Promise<void>((resolveClone, rejectClone) => {
            execFile(
                "git",
                ["clone", "--depth", "1", "--branch", "main", repositoryUrl, checkoutRoot],
                { encoding: "utf8", timeout: 30_000 },
                error => (error ? rejectClone(error) : resolveClone())
            );
        });
    } catch (error) {
...
export async function loadCatalog(sourceRoot = SOURCE_ROOT): Promise<Plugin[]> {
    ...
    for (const entry of entries) {
        ...
        try {
            packageJson = JSON.parse(await readFile(packagePath, "utf8")) as PluginPackage;
        } catch (error) {
            const message = error instanceof Error ? error.message : String(error);
