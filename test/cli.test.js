...
async function createSource(root, marker = "one", dependencies = {}) {
...
async function createTarget(root, { failFirstBuild = false, client = "equicord", version = "1.0.0" } = {}) {
    ...
    await writeFile(join(root, "build.mjs"), `import { access, readFile, rm, writeFile } from "node:fs/promises";
...
function captureOutput() {
    let text = "";
    return {
        output: { write(message) { text += message; } },
        read() { return text; }
    };
}
...
test("local build discovery finds clients hidden by a parent gitignore", async () => {
    ...
    assert.deepEqual(targets.map(target => [target.client, target.version]), [
        ["equicord", "1.15.0.2"],
        ["vencord", "1.15.0"]
    ]);
});
...
test("catalog tracks the current and latest main commits", async () => {
    ...
    assert.equal(pluginLabel(plugin, {
        mode: "copy",
...
test("plugin manager installs, updates, protects local changes, and removes", async () => {
    ...
    try {
        ...
        await assert.rejects(syncPlugins({
            userpluginsDir,
            registry,
            plugins: catalog,
            confirmDependencies: async () => true,
